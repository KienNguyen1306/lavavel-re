...
@php
    ...
    $data = Cache::remember('site.movies.latest', setting('site_cache_ttl', 5 * 60), function () {
        ...
        foreach ($lists as $list) {
            if (trim($list)) {
                $list = explode('|', $list);
                [$label, $relation, $field, $val, $sortKey, $alg, $limit, $link, $show_template] = array_merge($list, [
                    'Phim mới cập nhật',
                    '',
                    '',
                    'type',
...
@php
    $hero = $data[0] ?? null;
@endphp
@include('themes::themerrdyw.inc.section.section_hero_banner', [
    'item' => $hero
])
<div class="container">
    ...
    <div class="row">
        <div class="stui-pannel stui-pannel-bg clearfix">
            ...
            @endforeach --}}
             {{-- @dd($data) --}}
            @foreach (collect($data)->skip(1) as $item)
